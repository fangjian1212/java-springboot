...
import com.fangjian.xj.session.redis.SerializableValue;
 ...
 * {@link RedisOperationsSessionRepository.RedisSession#getMaxInactiveIntervalInSeconds()}
 * . For example:
 * </p>
 *
 * <pre>
...
public class RedisOperationsSessionRepository implements FindByIndexNameSessionRepository<RedisOperationsSessionRepository.RedisSession>, MessageListener {
	...
	public void onMessage(Message message, byte[] pattern) {
		...
		if(channel.startsWith(getSessionCreatedChannelPrefix())) {
			...
			// 修改点一
			//------------------------------------------------------------------------
			//防止反序列化的问题,created事件从redis加载session
	...
	public void handleCreated(String channel){
		String id = channel.substring(channel.lastIndexOf(":") + 1);
		RedisSession session = getSession(id);
		if (session != null) {
			publishEvent(new SessionCreatedEvent(this, session));
		}else {
			logger.warn("cannot find session by id=" + id);
		}
	}
	 ...
	 * {@link RedisOperationsSessionRepository.RedisSession#saveDelta()} is invoked
	...
	final class RedisSession implements ExpiringSession {
		...
		public Object getAttribute(String attributeName) {
			Object attribute = cached.getAttribute(attributeName);
			//------------------------------------------------------------------------
			// 修改点二
			//------------------------------------------------------------------------
			if(attribute instanceof SerializableValue){
				//因为延迟反序列化,session attribute 的类型可能是SerializableValue,这时需要反序列化
				return ((SerializableValue) attribute).deserialize();
			}else {
				return attribute;
			}
		}
