package com.fangjian.framework.utils.excel.imports;
...
public class ImportExcelUtil {
    ...
    public static <T> List<T> readExcel2007(MultipartFile file, Map<String, String> map, Class<T> clazz) {
        ...
        try (InputStream in = file.getInputStream()) {
            ...
            if (StringUtils.isEmpty(projectName) || !projectName.equals(map.get(PROJECT_NAME))) {
                throw new IllegalArgumentException("导入" + projectName + "项目不存在,请确认后重新导入");
            }
            if (StringUtils.isEmpty(projectName) || !layoutName.equals(map.get(LAYOUT_NAME))) {
                throw new IllegalArgumentException("导入" + layoutName + "户型不存在,请确认后重新导入");
            }
            // 循环输出表格中的第一行内容   表头
            ...
            if (row == null) {
                throw new IllegalArgumentException("表头信息异常");
            }
            //处理表头
            ...
            for (int i = sheet.getFirstRowNum() + 2; i <= sheet.getPhysicalNumberOfRows(); i++) {
                ...
                for (int j = row.getFirstCellNum(); j <= row.getPhysicalNumberOfCells(); j++) {
                    ...
                    String cellValue = getTableValue(cell);
    ...
    public static String getTableValue(XSSFCell cell) {
        String cellValue = "";
