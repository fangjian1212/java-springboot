package com.fangjian.framework.utils.excel.export;

import com.fangjian.framework.utils.excel.jxls2.JxlsUtil;
import com.fangjian.framework.utils.self.DateUtil;
...
public class ExportExcelUtil {
     ...
     * @param response
     */
    public static void exportExcel2(Map data, String fileName, String templateName, HttpServletRequest request, HttpServletResponse response) throws IOException {

        InputStream inputStream = null;
        ServletOutputStream outputStream = null;
        try {
            String filename = fileName + "-" + DateUtil.formatDateTime("yyMMddHHmmss", new Date()) + ".xlsx";
            setXlsHeader(response, request, filename);

            inputStream = JxlsUtil.getTemplateAsStream(templateName + ".xlsx");
            outputStream = response.getOutputStream();

            JxlsUtil.exportExcel(inputStream, outputStream, data);
            outputStream.flush();
            outputStream.close();
        } catch (IOException e) {
